            ...
            <form class="form-vertical" name="MetricsForm" role="form" method="POST">
                ...
                <fieldset>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="metric[name]" id="metric-name" required value="{{ request()->old('metric.name') }}" placeholder="{{ trans('forms.metrics.name') }}">
                    </div>
                    <div class="form-group">
                        <label for="metric-suffix">{{ trans('forms.metrics.suffix') }}</label>
                        <input type="text" class="form-control" name="metric[suffix]" id="metric-suffix" required value="{{ request()->old('metric.suffix') }}" placeholder="{{ trans('forms.metrics.suffix') }}">
                    </div>
                    <div class="form-group">
                        <label>{{ trans('forms.metrics.description') }}</label>
                        <div class='markdown-control'>
                            <textarea name="metric[description]" class="form-control" rows="5" placeholder="{{ trans('forms.metrics.description') }}">{{ request()->old('metric.description') }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" name="metric[default_value]" id="metric-default_value" value="{{ request()->old('metric.default_value') }}" placeholder="{{ trans('forms.metrics.default-value') }}" required>
                    </div>
                    <div class="form-group">
                        <label for="metric-places">{{ trans('forms.metrics.places') }}</label>
                        <input type="number" min="0" max="4" class="form-control" name="metric[places]" id="metric-places" required value="{{ request()->old('metric.places') }}" placeholder="{{ trans('forms.metrics.places') }}">
                    </div>
                    <div class="form-group">
                        <label for="metric-places">{{ trans('forms.metrics.threshold') }}</label>
                        <input type="number" min="0" max="10" name="metric[threshold]" value="{{ request()->old('metric.threshold') }}" class="form-control" required>
                    </div>
                    <div class="checkbox">
                        <label>
