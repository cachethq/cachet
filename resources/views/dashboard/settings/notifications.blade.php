...
<script>
    ...
    const mailEnabled = {{(Config::get('mail.driver') && Config::get('mail.from.address') && Config::get('mail.username') && Config::get('mail.password')) || (Config::get('mail.driver') === 'log') || Config::get('setting.enable_mail') ? 'true' : 'false'}};
    ...
    const mailText = (mailEnabled ? '{{trans('forms.settings.notifications.mail')}}' : '{{trans('forms.settings.notifications.notAvailable.mail', ['env' => implode(', ', array_filter([Config::get('mail.driver')? null :"MAIL_DRIVER", Config::get('mail.from.address')? null :"MAIL_ADDRESS", Config::get('mail.username')? null :"MAIL_USERNAME", Config::get('mail.password')? null :"MAIL_PASSWORD"]))])}}');
                ...
                <form id="settings-form" name="SettingsForm" class="form-vertical" role="form" action="{{ cachet_route('dashboard.settings', [], 'post') }}" method="POST">
                    ...
                    <fieldset>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="form-group">
                                    <label>{{ trans('forms.settings.notifications.rate_limit') }}</label>
                                    <input type="number" max="10000" name="sms_rate" class="form-control" value="{{ Config::get('setting.sms_rate', 5) }}" placeholder="{{ trans('forms.settings.notifications.rate_limit') }}">
                                </div>
                            </div>
                        </div>
                        <div class="row">
